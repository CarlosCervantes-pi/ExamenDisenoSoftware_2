# C4 - Diagrama de Código - Sistema de Reportes
...
sequenceDiagram
    ...
    participant System as ReportSystem
    participant GenFactory as ReportGeneratorFactory
    participant FmtFactory as FormatterFactory
    participant DelFactory as DeliveryFactory
    participant Generator as ReportGenerator
    participant Formatter as OutputFormatter
    participant Delivery as DeliveryMethod
    
    Client->>Facade: generate_sales_report_pdf_email(period, sales)
    Facade->>System: generate_report("sales", data, "pdf", "email")
    
    System->>GenFactory: create("sales")
    GenFactory-->>System: SalesReportGenerator
    
    System->>FmtFactory: create("pdf")
    FmtFactory-->>System: PDFFormatter
    
    System->>DelFactory: create("email")
    DelFactory-->>System: EmailDelivery
    
    System->>Generator: generate(data)
    Note over Generator: Template Method ejecuta:<br/>_build_header()<br/>_build_body()<br/>_build_footer()
    Generator-->>System: content (str)
    
    System->>Formatter: format(content)
    Formatter-->>System: formatted_content
    
    System->>Delivery: deliver(formatted_content, type, ext)
    Delivery-->>System: void
    
    System->>System: Guardar ReportRecord
    System-->>Facade: formatted_content
    Facade-->>Client: formatted_content
```